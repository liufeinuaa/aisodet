...
class RotatedRTMDetHead(RTMDetHead):
    ...
    Note: 修正了原始 mmrotate 库在开启 amp 训练的时候很容易 loss nan 的问题
    ...
    def __init__(self,
        ...
        # 新增loss_bbox_cfg来切换loss的一些参数
        # pdb.set_trace()
        self.loss_bbox_cfg = kwargs['loss_bbox']
...
# 手动关闭amp，使用回原本的 rotatediou 解决amp nan的问题, 测速表明是可以的---nice
            with torch.cuda.amp.autocast(enabled=False):
                # pdb.set_trace()
                if self.loss_bbox_cfg.type == "GDLoss_v3":
                    ...
                    loss_bbox = self.loss_bbox( # 开启amp 很容易就 loss nan
                        pos_decode_bbox_pred,
                        pos_decode_bbox_targets,
                        weight=pos_bbox_weight,
                        avg_factor=1.0)
        ...
        if torch.isnan(loss_bbox): #增加对 loss nan 的检测
            pdb.set_trace()
...
# 直接在head中对开启amp可能会nan 的处理部分，手动关闭amp
            with torch.cuda.amp.autocast(enabled=False):
        ...
        if self.loss_angle is not None:
            ...
            # 增加返回 model forword 的 cls scores 用于 esrtmdet中 mask的生成
            return dict(loss_cls=losses_cls, loss_bbox=losses_bbox, cls_scores=cls_scores)
    ...
    def _predict_by_feat_single(self,
        ...
        # 新增的用于test中的对det检测的结果放大2倍以便提交DOTA 官方test服务器进行测试
        chg_scale_factor = cfg.test_bbox_up2x if 'test_bbox_up2x' in cfg else False # 设置默认值，方便修改
        if chg_scale_factor:
            # pdb.set_trace()
            img_meta['scale_factor'] = [s*0.5 for s in img_meta['scale_factor']] # 将最后出来的bbox结果放大2倍
